// ==========================================
// MESIN LOGIKA GUDANG (V.19.0 - DUAL CAMERA/GALLERY & CLEAN BULK UI)
// ==========================================
...
let pendingAddFotos = []; // Array khusus penampung foto baru
...
function toggleAdminMode() {
    if (isAdminMode) { if(confirm("Tutup akses Admin? Memori PIN akan dihapus.")) { localStorage.removeItem("AV_INVENTORY_PIN"); userPin = ""; checkAdminStatus(); showToast("Mode Read-Only aktif."); applyFilters(); }
    } else { let input = prompt("Masukkan PIN Kapten / Admin:"); if (input) { let pinAttempt = input.trim().toLowerCase(); if (VALID_PINS.includes(pinAttempt)) { userPin = pinAttempt; localStorage.setItem("AV_INVENTORY_PIN", userPin); checkAdminStatus(); showToast("Akses Admin Terbuka!"); applyFilters(); } else { alert("⛔ AKSES DITOLAK! PIN salah."); } } }
}
...
function getFilteredData() { const q = document.getElementById("searchInput").value.toLowerCase(); return allItems.filter(i => { const matchQ = i.nama_barang.toLowerCase().includes(q) || (i.kode_barang||"").toLowerCase().includes(q) || (i.kode_wadah||"").toLowerCase().includes(q); let stat = i.status_digunakan || 'Di Gudang'; if(stat === 'FALSE') stat = 'Di Gudang'; let matchPill = (activeFilterPill === 'all') ? true : (stat === activeFilterPill); return matchQ && matchPill; }); }
...
function render(data) {
    ...
    data.forEach(item => {
        const card = document.createElement("div"); const isSelected = selectedRows.has(item.row_index); let stat = item.status_digunakan || "Di Gudang"; if(stat === 'FALSE') stat = "Di Gudang"; let lok = item.lokasi || "Gudang KC";
        ...
        if (currentViewMode === 'grid') {
            card.className = "mission-card " + (isSelected ? "selected " : "") + (stat === 'Akan Dibawa' ? "card-siap-dibawa " : ""); card.innerHTML = `${isSelected ? '<div class="card-check">✓</div>' : ''}<div style="position:relative;"><img src="${imageSrc}" class="card-img" loading="lazy"><div style="position:absolute; bottom:12px; right:4px;"><span class="badge-qty">Qty: ${item.jumlah || 0}</span></div></div><h4 class="card-title">${item.nama_barang}</h4>${timeBadge} <div class="card-codes" style="display:flex; align-items:center;">${kodeBadge} ${kondisiBadge}</div> <div>${badgeLokasiHtml}</div>`;
        } else {
            card.className = "list-item " + (isSelected ? "selected " : "") + (stat === 'Akan Dibawa' ? "card-siap-dibawa " : ""); card.innerHTML = `${isSelected ? '<div class="card-check" style="top:50%; transform:translateY(-50%); right:15px;">✓</div>' : ''}<img src="${imageSrc}" class="list-img" loading="lazy"><div class="list-info"><h4 class="list-title">${item.nama_barang}</h4>${timeBadge}<div style="display:flex; gap:6px; flex-wrap:wrap; align-items:center; font-size:10px;">${kodeBadge} ${kondisiBadge} ${badgeLokasiHtml} <span class="badge-qty">Qty: ${item.jumlah || 0}</span></div></div>`;
        }
...
function openDetailModal(item) {
    ...
    let optionsLokasi = `<option value="Gudang KC" ${lok.includes('Gudang') ? 'selected':''}>🏢 Gudang KC</option><option value="Dalam Perjalanan" ${lok === 'Dalam Perjalanan' ? 'selected':''}>🚚 Dalam Perjalanan</option><option value="Di Lokasi Event" ${lok.includes('Event') ? 'selected':''}>📍 Di Lokasi Event</option>`; let optionsStatus = `<option value="Di Gudang" ${stat === 'Di Gudang' ? 'selected':''}>📦 Standby / Di Gudang</option><option value="Akan Dibawa" ${stat === 'Akan Dibawa' ? 'selected':''}>🛒 Akan Dibawa (Packing)</option><option value="Sedang Dipakai" ${stat === 'Sedang Dipakai' ? 'selected':''}>🔌 Sedang Dipakai / Aktivasi</option><option value="Sedang Diservis" ${stat === 'Sedang Diservis' ? 'selected':''}>🛠️ Sedang Diservis</option>`;
...
// ==========================================
// PENGELOLA FOTO BARU (DUAL BUTTON)
// ==========================================
...
function handleNewFotos(input) {
    if (!input.files || input.files.length === 0) return;
    for (let i = 0; i < input.files.length; i++) { if (pendingAddFotos.length < 3) pendingAddFotos.push(input.files[i]); }
    input.value = ""; renderPreviewAddFotos();
}
...
async function submitNewItem(e) {
    ...
    try {
        let base64Fotos = ["", "", ""]; 
        let maxFiles = Math.min(pendingAddFotos.length, 3); 
...
// ==========================================
// KERANJANG (CLEAN BULK UPDATE & TOMBOL PILIH SEMUA DI BAWAH)
// ==========================================
function toggleBulkMode() {
    isBulkMode = !isBulkMode; let bar = document.getElementById("bulkBar");
    if(!bar) { 
        document.body.insertAdjacentHTML('beforeend', `<div id="bulkBar" class="bulk-bar"><span id="bulkCount" class="bulk-info" style="font-weight:bold;">0 Terpilih</span><div style="display:flex; gap:8px;"><button onclick="selectAllVisible()" style="background:#e2e8f0; color:#334155; border:none; padding:8px 12px; border-radius:8px; font-weight:bold; font-size:12px;">☑️ Semua</button><button onclick="openBulkUpdateModal()" class="btn-bulk-process" style="padding:8px 12px; font-size:12px;">Ubah Massal</button></div></div>`); 
        bar = document.getElementById("bulkBar"); 
    }
    const btnMode = document.getElementById("btnBulkMode");
    if (isBulkMode) { btnMode.innerHTML = `❌ Batal Massal`; btnMode.style.background = "#ef4444"; bar.classList.add("active"); } 
    else { btnMode.innerHTML = `🛒 Mode Massal`; btnMode.style.background = "#ea580c"; bar.classList.remove("active"); selectedRows.clear(); document.getElementById("bulkCount").innerText = `0 Terpilih`; }
    applyFilters(); 
}
...
function openBulkUpdateModal() {
    ...
    const modalHtml = `<div id="bulkModal" class="modal-overlay active"><div class="modal-content" style="max-width:350px; padding:20px; background:white; border-radius:15px; position:relative;"><button onclick="document.getElementById('bulkModal').remove()" style="position:absolute; top:15px; right:15px; border:none; background:#f1f5f9; width:30px; height:30px; border-radius:50%; font-weight:bold;">✕</button><h3 style="margin-top:0;">Ubah Massal (${selectedRows.size} Alat)</h3><div style="text-align:left; margin-bottom:15px;"><label style="font-size:11px; font-weight:bold; color:gray; display:block; margin-bottom:4px;">📍 Ubah Lokasi Saat Ini (Kolom O):</label><select id="bulkNewLokasi" style="width:100%; padding:10px; border-radius:8px; border:1px solid #ccc; font-weight:bold;"><option value="TETAP">-- Jangan Ubah Lokasi --</option><option value="Gudang KC">🏢 Gudang KC</option><option value="Dalam Perjalanan">🚚 Dalam Perjalanan</option><option value="Di Lokasi Event">📍 Di Lokasi Event</option></select></div><div style="text-align:left; margin-bottom:15px;"><label style="font-size:11px; font-weight:bold; color:gray; display:block; margin-bottom:4px;">🔌 Ubah Status Alat (Kolom R):</label><select id="bulkNewStatus" style="width:100%; padding:10px; border-radius:8px; border:1px solid #ccc; font-weight:bold;"><option value="TETAP">-- Jangan Ubah Status --</option><option value="Akan Dibawa">🛒 Akan Dibawa (Packing)</option><option value="Sedang Dipakai">🔌 Sedang Dipakai / Aktivasi</option><option value="Di Gudang">📦 Standby / Di Gudang</option></select></div><div style="font-size:10px; color:gray; text-align:left; margin-bottom:15px;"><i>*Tips: Untuk "Kembalikan ke Gudang", ubah Lokasi jadi "Gudang KC" dan Status jadi "Di Gudang".</i></div><button onclick="processBulkUpdate(this)" style="width:100%; padding:12px; background:#ea580c; color:white; border:none; border-radius:8px; font-weight:bold;">PROSES UPDATE LOKASI & STATUS</button></div></div>`; document.body.insertAdjacentHTML('beforeend', modalHtml);
}
async function processBulkUpdate(btn) {
    const newLokasi = document.getElementById("bulkNewLokasi").value; const newStatus = document.getElementById("bulkNewStatus").value; if (newLokasi === "TETAP" && newStatus === "TETAP") { alert("Pilih minimal satu perubahan (Lokasi atau Status)!"); return; }
    btn.disabled = true; btn.innerText = "MEMPROSES... (JANGAN DITUTUP)";
    try { const payload = { action: "update_status_lokasi", pin: userPin, rows: Array.from(selectedRows), new_lokasi: newLokasi !== "TETAP" ? newLokasi : null, new_status: newStatus !== "TETAP" ? newStatus : null }; const response = await fetch(SCRIPT_URL, { method: "POST", body: JSON.stringify(payload) }); const data = await response.json(); if(data.status === "success") { document.getElementById('bulkModal').remove(); toggleBulkMode(); loadData(); showToast("✅ Update massal berhasil!"); } else { alert("Gagal:\n" + data.message); } } catch (e) { alert("Error Sistem:\n" + e.message); } finally { btn.disabled = false; btn.innerText = "PROSES UPDATE"; }
}
...
async function submitEditFull(e) {
    ...
    try { let finalFotos = ["", "", ""]; for(let i=0; i<3; i++) { let existVal = document.getElementById("existingId" + i).value; let fileInput = document.getElementById("editFoto" + i); if (existVal === "NEW_BASE64" && fileInput.files.length > 0) { finalFotos[i] = await compressImage(fileInput.files[0]); } else if (existVal && existVal.length > 5) { finalFotos[i] = existVal; } else { finalFotos[i] = ""; } }
...
function openScannerModal() {
    ...
    html5QrcodeScanner.render((decodedText) => {
        ...
        } else { closeScannerModal(); document.getElementById('searchInput').value = scanResult; applyFilters(); const foundItem = allItems.find(i => (i.kode_barang||"").toString().toLowerCase() === scanResult.toLowerCase() || (i.kode_wadah||"").toString().toLowerCase() === scanResult.toLowerCase()); if (foundItem) openDetailModal(foundItem); }
    });
}
